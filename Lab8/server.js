...
app.get('/card', function(req, res) {  
    ...
    client.connect(error => {
      ...
      ids.find({}).sort({name: 1}).toArray(function(err, result) {
...
app.get('/card/name/:name', function(req, res) {  
    let n = req.params.name.split('_').join(' ');
    //console.log(req.body);
    client.connect(error => {
      console.log("Connected to MongoDB server...");
      const ids = client.db("lab6").collection("cards");
      ids.find({name: n}).toArray(function(err, result) {
        console.log("find query executed...");  
        if (result == undefined || result.length == 0) {
            console.log("could not GET document ");
            res.json({}).status(404);
